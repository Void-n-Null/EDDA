...
class MessageHandler:
    ...
    async def _handle_set_volume(self, msg: ServerMessage) -> None:
        """Handle volume control via ALSA (amixer) or PulseAudio (pactl)."""
        vol = msg.volume
        if vol is None:
            return

        # Try ALSA first (more reliable on headless Pi), then PulseAudio
        success = await self._try_alsa_volume(vol)
        if not success:
            success = await self._try_pulseaudio_volume(vol)

        if not success:
            print("[VOLUME] All methods failed")

    async def _try_alsa_volume(self, vol) -> bool:
        """Try setting volume via ALSA (amixer)."""
        try:
            if vol.relative:
                # ALSA uses different syntax for relative
                if vol.volume >= 0:
                    volume_arg = f"{vol.volume}%+"
                else:
                    volume_arg = f"{abs(vol.volume)}%-"
            else:
                volume_arg = f"{max(0, min(100, vol.volume))}%"

            # Try common ALSA control names
            controls = ["Master", "PCM", "Headphone", "Speaker"]

            for control in controls:
                result = subprocess.run(
                    ["amixer", "sset", control, volume_arg],
                    capture_output=True,
                    text=True,
                    timeout=5,
                )
                ...
                if result.returncode == 0:
                    print(f"[VOLUME] ALSA ({control}) set to {volume_arg}")
                    return True
            ...
            print("[VOLUME] ALSA command timed out")
            return False
        except Exception as e:
            print(f"[VOLUME] ALSA error: {e}")
            return False

    async def _try_pulseaudio_volume(self, vol) -> bool:
        """Try setting volume via PulseAudio (pactl)."""
        try:
            if vol.relative:
                sign = "+" if vol.volume >= 0 else ""
                volume_arg = f"{sign}{vol.volume}%"
            else:
                volume_arg = f"{max(0, min(100, vol.volume))}%"

            # Set environment for PulseAudio (helps with headless/service contexts)
            env = os.environ.copy()
            env["XDG_RUNTIME_DIR"] = f"/run/user/{os.getuid()}"

            result = subprocess.run(
                ["pactl", "set-sink-volume", "@DEFAULT_SINK@", volume_arg],
                capture_output=True,
                text=True,
                timeout=5,
                env=env,
            ...
            if result.returncode == 0:
                print(f"[VOLUME] PulseAudio set to {volume_arg}")
                return True
            else:
                print(f"[VOLUME] PulseAudio failed: {result.stderr.strip()}")
                return False
        except FileNotFoundError:
            return False
        except subprocess.TimeoutExpired:
            print("[VOLUME] PulseAudio command timed out")
            return False
        except Exception as e:
            print(f"[VOLUME] PulseAudio error: {e}")
            return False
